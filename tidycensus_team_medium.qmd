...
author: "Amber Camp" 
collaborators: "Johnny, Calvin, Karl"
date: "`r Sys.Date()`"
format: html
editor: visual
toc: true
...
```{r}
# get the data
oahu_median_age <- get_acs(
  geography = "tract",
  variables = c(median_age = "DP05_0017E"),  # Median age
  state = "HI",
  county = "Honolulu",
  geometry = TRUE,
  output = "wide",
  year = 2023) %>%
  dplyr::filter(!GEOID %in% c("15003981200","15003981900"))

# map it
oahu_median_age_map <- ggplot(oahu_median_age) +
  geom_sf_interactive(aes(
    fill = median_age,
    tooltip = paste(NAME, ": ", round(median_age, 1), " years"),
    data_id = NAME)) +
  scale_fill_viridis_c(option = "plasma") +
  labs(
    title = "Median Age by Census Tract — Honolulu County (2023)",
    fill = "Median Age") +
  theme_void()

girafe(ggobj = oahu_median_age_map, width = 700, height = 400)
```
...
oahu_veterans_geo <- get_acs(
  geography = "tract",
  variables = c(
    total_pop_18plus = "C21007_001E",
    total_veterans = "C21007_002E",
    veterans_below_poverty = "C21007_003E",
    nonveterans_below_poverty = "C21007_007E"),
  state = "HI",
  county = "Honolulu",
  geometry = TRUE,
  output = "wide",
  ...
  janitor::clean_names() %>%
  dplyr::mutate(
    pct_vets_poverty = round(((veterans_below_poverty / total_veterans) * 100),2),
    pct_nonvets_poverty = round(((nonveterans_below_poverty /
                           (total_pop_18plus - total_veterans)) * 100),2)) %>%
  dplyr::select(
    name, geoid,
    total_pop_18plus, total_veterans,
    veterans_below_poverty, nonveterans_below_poverty,
    pct_vets_poverty, pct_nonvets_poverty,
    geometry)
```
...
```{r, message=FALSE, warning=FALSE, results='hide'}
oahu_veteran_poverty_map <- ggplot(oahu_veterans_geo) +
  geom_sf_interactive(
    aes(
      fill = pct_vets_poverty,
      tooltip = paste(name, ": ", pct_vets_poverty, "%"),
      data_id = name)) +
  scale_fill_viridis_c(option = "mako", direction = -1) +
  theme_void() 

girafe(ggobj = oahu_veteran_poverty_map, width = 700, height = 400)
```
...
oahu_vets_pov_2014_2023 <- purrr::map_dfr(
  years,
  ~ get_acs(
      geography = "county",
      variables = c(vets_below_pov = "C21007_003E"),  # count (E) + MOE (M)
      state = "HI",
      county = "Honolulu",
      year = .x,
      survey = "acs5"
    ) |>
    dplyr::mutate(year = as.integer(.x))
)
```

```{r}
ggplot(oahu_vets_pov_2014_2023, aes(x = year, y = estimate, group = 1)) +
  ...
  theme_minimal() +
  labs(
    title = "Veterans Below Poverty — Honolulu County (ACS 5-year estimates)",
    x = "ACS Release Year (5-year window)",
    y = "Veterans Below Poverty (count)",
    caption = "Shaded area represents margin of error around the estimate"
  )
```
...
```{r, message=FALSE, warning=FALSE, results='hide'}
oahu_vets <- oahu_veterans_geo %>%
  ggplot(aes(x = total_veterans, y = veterans_below_poverty)) +
  geom_point(color = "orange", alpha = 0.8, size = 3) +
  ...
  labs(
    title = "Veterans in Poverty — O‘ahu (Honolulu County)",
    subtitle = "ACS 5-Year Estimates, 2023 Release",
    x = "Total Veterans (per Census Tract)",
    y = "Veterans Below Poverty",
    caption = "Shaded area represents margin of error around the estimate")
...
Veteran poverty on Oʻahu varies by area but has stayed fairly steady over time. Tracts with more veterans tend to have slightly higher numbers living in poverty, though the relationship isn’t very strong.
