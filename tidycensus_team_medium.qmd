...
oahu_veterans_geo <- get_acs(
  ...
  janitor::clean_names() %>%
  dplyr::mutate(
    pct_vets_poverty = (veterans_below_poverty / total_veterans) * 100,
    pct_nonvets_poverty = (nonveterans_below_poverty /
                           (total_pop_18plus - total_veterans)) * 100
  ) %>%
  dplyr::select(
    name, geoid,
...
oahu_veteran_poverty_map <- ggplot(oahu_veterans_geo) +
  geom_sf_interactive(
    aes(
      fill = veterans_below_poverty,
      tooltip = paste(name, ": ", veterans_below_poverty, "%"),
      data_id = name)) +
